...
import castleHeaderBackground from "../public/img/castle-header-background.jpg";
...
const DramaPage = ({ statusCode, dramaData }) => {
  ...
  const backgroundImage = dramaData?.image ? "https://image.tmdb.org/t/p/original/" + dramaData.image : castleHeaderBackground.src;
  ...
  return (
    <>
      ...
      <div className="bg-[left_top] bg-[length:1200px] lg:bg-cover min-h-screen bg-no-repeat	"
           style={{ background: `linear-gradient(90deg, rgb(16, 24, 40) 10%, rgb(16 24 40 / 78%) 50%, rgb(22 32 54 / 21%) 99%),linear-gradient(0deg, rgb(16 24 40) 10%, rgb(22 32 54 / 0%) 99%), url('${backgroundImage}')` }}>
        <DramaHeader/>
        <main className="w-4/5 m-0 m-auto max-w-screen-xl">
          <article className="container mx-auto md:w-4/6 md:mx-0">
            <DramaDetails name={dramaData.name}
          ...
          {dramaData.recommendations && <aside>
            <h2 className="text-primary-50 text-sm font-medium mt-16">You May Also Like</h2>
            {dramaData.recommendations.length === 0 ? <p className="mt-4">No recommendations found</p> :
              <div className="grid grid-cols-1 sm:grid-cols-2 sm:gap-8 lg:grid-cols-3">

                {dramaData.recommendations.map(drama => <CatalogueResultsItem
                  key={drama.id}
                  id={drama.id}
...
export async function getServerSideProps(params) {
  ...
  try {
    ...
    if (statusCode === 200 && data && data.name) {
      const asianTVRecommendations = filterNonAsianResults(data.recommendations);
      const threeRecommendations = asianTVRecommendations ? asianTVRecommendations.results.slice(0, 3) : [];
